                    ...
                    try{
                        ...
                        if(eventData){
                            ...
                            if(OnlyMOBMMarketIdsArr.length !== 0 && !issportHRGH){
                                ...
                                liveMatchCheckMarket = fetchMarketData2.find(item => item.marketName.trim() == 'Match Odds' && item.status !== "CLOSED")
                                liveMatchCheckMarket2 = fetchMarketData2.find(item => item.marketName.trim() == 'Bookmaker' && item.status !== "CLOSED")
                                if(!liveMatchCheckMarket2 || eventData.sportId == "500"){
                                    liveMatchCheckMarket2 = fetchMarketData2.find(item => item.marketName.trim() == "Bookmaker 0 Commission" && item.status !== "CLOSED")
                                    if(!liveMatchCheckMarket2 && eventData.sportId == "500"){
                                        liveMatchCheckMarket3 = fetchMarketData2.find(item => item.bettingType == "BOOKMAKER" && item.status !== "CLOSED")
                                    }
                                }
                                liveMatchCheckMarket4 = fetchMarketData2.find(item => item.marketType == 'TOURNAMENT_WINNER' && item.catalogue.status !== "CLOSED")
