...
module.exports = () => {
    cron.schedule('*/10 * * * *', async() => {
        try{
                let cricketEventIds = await client.get('crone_getEventIds_Cricket_diamond_UPD'); 
                if(!cricketEventIds){
                    cricketEventIds = await client.get('crone_getEventIds_Cricket_diamond'); 
                }
                let cricketEventIds2 = await client.get('crone_CricketliveEventIds_diamond_UPD'); 
                ...
                for(let i = 0;i<cricketEventIds3.length;i++){
                    let marketIds = await client.get(`cricketFanctMarketIds_${cricketEventIds3[i]}_diamond`)
